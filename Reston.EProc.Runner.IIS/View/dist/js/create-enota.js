...
function loadListKandidat(Id, status,xisTEam) {
    $(".listkandidat").html("");
        $.ajax({
            method:"POST",
            url: "Api/ENota/GetKandidats?PId=" + Id
        }).done(function (data) {
...
function silensave(pengadaanHeader,attr1) {
    ...
    $.ajax({
        method: "POST",
        url: "Api/ENota/save?status=Draft",
        dataType: "json",
...
function save(eNota,attr1,status) {
    var data = {};
    data.ENota = eNota;
    eNota.IsDraft = status;
    waitingDialog.showloading("Proses Harap Tunggu");
    $.ajax({
        method: "POST",
        url: "Api/ENota/save?status=" + status,
        dataType: "json",
        data: JSON.stringify(data),
        contentType: 'application/json; charset=utf-8'
    }).done(function (data) {
...
function getHeaderPengadaan() {
    var viewENota = {};
    viewENota.Subject = $("[name=Subject]").val();
    viewENota.DocumentNo = $("[name=DocumentNo]").val();
    viewENota.WorkUnitCode = $("[name=WorkUnitCode]").val();;
    viewENota.InternalRefNo = $("[name=InternalRefNo]").val();
    viewENota.HPSAmount = $("[name=HPSAmount]").val();
    viewENota.HPSCurrency = $("[name=HPSCurrency]").val();
    viewENota.IsDraft = $("[name=IsDraft]").val();
    viewENota.Owner = $("[name=Owner]").val();
    if ($("#eNotaId").val() != "") viewENota.Id = $("#enotaId").val();
    return viewENota;
}
...
function addPersonilz(item, el) {
    ...
    objParticipant.ENotaId = $("#enotaId").val();
...
function addPersonil(item, el) {
    ...
    objPersonilPengadaan.PengadaanId = $("#enotaId").val();
...
function loadHeaderPengadaan(viewENota) {
    var xPermision = viewENota.isCreated == 1 ? 1 : viewENota.isTEAM == 1 ? 1 : 0;
    LoadKriteriaPembobotan(viewENota.Id, xPermision);
    if (viewENota.Status == 0 || viewENota.Status == 10) {
        if (viewENota.isPIC == 1 && viewENota.Status != 10) {
            $(".SimpanAjukan").show();
            
        }
        $("#isTeam").val(viewENota.isTEAM);
        $("#isCreated").val(viewENota.isCreated);
        $("#State").val(viewENota.Status);
        if (viewENota.isTEAM == 1 || viewENota.isCreated == 1)
            $(".Simpan").show();
        if (viewENota.isTEAM == 0 && viewENota.isCreated==0) {
        ...
        if ((viewENota.isPIC == 1 || viewENota.isCreated)) {
            $(".Hapus").show();
        }
    }

    if (viewENota.Status == 1) {
        //buat_hps
        $("input").attr("disabled", "disabled");
        $("select").attr("disabled", "disabled");
        $("textarea").attr("disabled", "disabled");
        $("#buat_hps").text("Lihat HPS");
        $("#buat_hps").attr("status", viewENota.Status);
        $(".SimpanAjukan").show();
    }

    if (viewENota.Status == 10) {
        
        $(".SimpanAjukan").hide();
    }

    if (viewENota.Approver == 1) {
        $(".Setujui").show();
        $(".Tolak").show();
    }
        
    $("[name=Subject]").val(viewENota.Subject);
    $("[name=DocumentNo]").val(viewENota.DocumentNo) ;
    $("[name=InternalRefNo]").val(viewENota.InternalRefNo);    
    $("[name=HPSAmount]").val(viewENota.HPSAmount) ;
    $("[name=HPSCurrency]").val(viewENota.HPSCurrency);
    $("[name=IsDraft]").val(viewENota.IsDraft);
    $("[name=Owner]").val(viewENota.PeriodeAnggaran);
    $("[name=WorkUnitCode]").val(viewENota.JenisPembelanjaan);
    $("#enotaId").val(viewENota.Id);

    aturanPenawaranView();
    aturanPengadaanView();

    loadListKandidat(viewENota.Id, viewENota.Status, xPermision);
    LoadListPersonil(viewENota.PersonilPengadaans, viewENota.Status, xPermision);
    loadKualifikas(viewENota.KualifikasiKandidats)
    hitungHPS(id_rks);
...
function loadData(id_rks) {
    $.ajax({
        method: "POST",
        url: "Api/ENota/detailENota?Id=" + id_rks
    }).done(function (data) {  
data = DOMPurify.sanitize(data);	
...
function addVendor(item) {
    var kandidat = {};
    kandidat.PengadaanId = $("#enotaId").val();
...
function hapusKandidat(Id,_this) {
    ...
    $.ajax({
        method: "POST",
        url: "Api/ENota/deleteKandidat?Id="+Id
...
function hapusPersonil(Id, _this) {
    ...
    $.ajax({
        method: "POST",
        url: "Api/ENota/deleteParticipant?Id=" + Id
...
function viewFile(File) {
    $.ajax({
        url: "Api/ENota/ViewFile?file=" + File,
...
function viewDocDetail(File) {
    $.ajax({
        url: "api/ENota/OpenFile?file=" + File
    });
}
...
function LoadKriteriaPembobotan(PengadaanId,xIsTeam) {
    ...
    $.ajax({
        method: "POST",
        url: "Api/ENota/getKriteriaPembobotan?PengadaanId=" + PengadaanId,
...
function addPembobotanPengadaan(el) {
    ...
    oData.Bobot = el.val();
    oData.PengadaanId = $("#enotaId").val();
    $.ajax({
        method: "POST",
        url: "Api/ENota/addPembobotanPengadaan",
        dataType: "json",
...
function addLstPembobotanPengadaan() {
    ...
    $.ajax({
        method: "POST",
        url: "Api/ENota/addLstPembobotanPengadaan",
        dataType: "json",
...
$(function () {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function (dialog) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: "Api/ENota/deleteParticipant?Id=" + $("#enotaId").val()
                    }).done(function (data) {
						data = DOMPurify.sanitize(data);
                        window.location.replace(HOME_PAGE + "/enota-list.html");
                        waitingDialog.hideloading();
                    });
    ...
    $("#buat_hps").on("click", function () {
        ...
        else window.location.replace(HOME_PAGE + "/rks.html#" + parseInt($("#enotaId").val()));
    });
    ...
    if (isGuid(id_rks)) {
        ...
        if (isGuid($("#enotaId").val())) {
            window.location.hash = $("#enotaId").val();
   ...
   // }
    ...
    $("#HapusFile").on("click", function () {
        ...
        $.ajax({
            method: "POST",
            url: "Api/ENota/deleteDokumen?Id=" + FileId
        }).done(function (data) {
			data = DOMPurify.sanitize(data);
            if (data.Id == "1") {
    ...
    $(".Setujui").on("click", function () {
        $.ajax({
            method:"POST",
            url: "Api/ENota/persetujuan?Id=" + parseInt($("#pengadaanId").val()) ,
            success: function (data) {
				data = DOMPurify.sanitize(data);
                if (data.status == 200) {                    
    ...
    $(".checkbox-kualifikasi").on("click", function () {
        ...
        if ($(this).is(':checked')) {
            ...
            $.ajax({
                method: "POST",
                url: "Api/ENota/saveKualifikasiKandidat",
                dataType: "json",
                data: JSON.stringify(data),
                contentType: 'application/json; charset=utf-8',
            ...
            $.ajax({
                method: "POST",
                url: "Api/ENota/deleteKualifikasiKandidat?Id="+$(this).attr("attrId"),
                success: function (data) {
					data = DOMPurify.sanitize(data);
                    $(ell).attr("attrId", "");
                },
    ...
    $("#downloadFile").on("click", function () {
        var tipe = $(this).parent().parent().parent().parent().attr("attr1");
        var FileId = $(this).parent().parent().parent().parent().attr("FileId");
        downloadFileUsingForm("/api/ENota/OpenFile?Id=" + FileId);
    });
...
function renderDokumenDropzone(myDropzone,tipe) {
    $.ajax({
        url: "Api/ENota/getDokumens?Id=" + $("#enotaId").val() + "&tipe=" + tipe,
        success: function (data) {
			data = DOMPurify.sanitize(data);
            for (var key in data) {
                var file = {

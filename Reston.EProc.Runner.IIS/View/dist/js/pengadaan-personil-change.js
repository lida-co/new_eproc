...
function loadHeaderPengadaan(viewPengadaan) {
    ...
    if (viewPengadaan.Status != 0 || viewPengadaan.Status == 0 || viewPengadaan.Status == 10) {
        ...
        //if (viewPengadaan.isTEAM == 1 || viewPengadaan.isCreated == 1) isMasterUser
        if (isMasterUser == 1) 
            $(".Simpan").show();
...
function hapusPersonil(Id, _this) {
    BootstrapDialog.show({
        title: 'Konfirmasi',
        message: 'Silahkan Isi Alasan Perubahan Personil, Harus di Isi !' + '<textarea class="form-control" id="alasan"></textarea>',
        buttons: [{
            label: 'Konfirmasi',
            action: function (dialog) {
                if ($("#alasan").val() == '' || $("#alasan").val() == "") {
                    dialog.close();
                    $("#modal-alert-alasan").modal({ backdrop: 'static', keyboard: false });
                } else {
                    dialog.close();
                    hapusPersonilwithNote(Id, _this);
                }
            }
        }]
    });
...
function hapusPersonilwithNote(Id, _this) {
    $.ajax({
        method: "POST",
        url: "Api/PengadaanE/DeletePersonilMasterUser?Id=" + Id + "&PengadaanId=" + $("#pengadaanId").val() + "&alasan=" + $("#alasan").val(),
    }).done(function (data) {
        if (data.status == 200) {
            // _this.parent().parent().parent().remove();
            _this.parent().remove();
            $("#modal-hapus-person").modal({ backdrop: 'static', keyboard: false });
        }
        else {
            alert("error");
        }
...
function addPersonil(item, el) {
    ...
    $.ajax({
        method: "POST",
        url: "Api/PengadaanE/SavePersonilMasterUser?PengadaanId=" + $("#pengadaanId").val(),
        dataType: "json",
        ...
        if (data.status == 200) {
            html = '<a class="btn btn-app">' +
                ...
                '</a>';
            $(el).append(html);
            $("#modal-tambah-person").modal({ backdrop: 'static', keyboard: false });
        }
        else {
            alert("Perubahan personil gagal");
        }
    });
...
function tutup() { window.location.reload(); }
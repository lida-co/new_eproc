...
function generateTable(pengadaanId) {
    $.ajax({
        ...
        url: 'Api/PengadaanE/getRKSPenilaian?PId=' + DOMPurify.sanitize(pengadaanId),
        dataType: 'json',
        success: function (data) {
            ...
            for (var j in data.vendors) {
                str_row += '<th>' + DOMPurify.sanitize(data.vendors[j].nama) + '</th>';
            }
            ...
            for (var i in data.hps) {//accounting.formatNumber(totalHarga, { thousand: "." })
                ...
                if (data.hps[i].level == 0)
                    str_row += '<td>' + DOMPurify.sanitize(data.hps[i].judul) + '</td>';
                else str_row += '<td></td>';
                str_row += '<td>' + DOMPurify.sanitize(data.hps[i].item) + '</td>';
                if (data.hps[i].harga > 0) {                    
                    if (data.hps[i].satuan != null) str_row += '<td>' + DOMPurify.sanitize(data.hps[i].satuan) + '</td>';
                    else str_row += '<td></td>';
                    if (data.hps[i].jumlah > 0) str_row += '<td>' + accounting.formatNumber(DOMPurify.sanitize(data.hps[i].jumlah), { thousand: ".", decimal: ",", precision: 2 }) + '</td>';
                    else str_row += '<td></td>';
                    if (data.hps[i].harga > 0) str_row += '<td class="rata_kanan">' + accounting.formatNumber(DOMPurify.sanitize(data.hps[i].harga), { thousand: ".", decimal: ",", precision: 2 })  + '</td>';
                    else str_row += '<td></td>';
                }
                else {
                    str_row += '<td></td>' +
...
function setListHargaVendor(vendor) {
	...
	if(vendor){				
		for (var i in vendor) { 
            str_vendor += '<div class="col-md-2 " vendorId="' + DOMPurify.sanitize(vendor[i].VendorId)+ '"><div class="box box-primary box-folder-vendor"><div class="box-tools pull-right vendor-check-box"><div class="row" style="margin-right:3px;"><span class="fa fa-eye"></span> ' +

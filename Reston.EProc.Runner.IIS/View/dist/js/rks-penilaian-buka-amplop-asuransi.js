...
function generateTable(pengadaanId) {
    $.ajax({
        ...
        url: 'Api/PengadaanE/getRKSPenilaianAsuransi?PId=' + pengadaanId,
        dataType: 'json',
...
$(function () {
    if (isGuid(id_pengadaan)) {
        ...
        $("body").on("click", "#CetakWord", function () {
            downloadFileUsingForm("/api/report/CetakRKSPenilaianAsuransiAll?Id=" + $("#pengadaanId").val());
        });

        $("body").on("click", "#CetakXls", function () {
            downloadFileUsingForm("/api/report/CetakRKSPenilaianAsuransiAllXls?Id=" + $("#pengadaanId").val());
        });

...
function generateTable(pengadaanId,vendorId) {
    $.ajax({
        ...
        url: 'Api/PengadaanE/getRKSKlarifikasiPenilaianVendor?PId=' + DOMPurify.sanitize(pengadaanId) + "&VendorId=" + DOMPurify.sanitize(vendorId),
        dataType: 'json',
        success: function (data) {
            bcde = data;

            // clear old table if exists
            const tableEl = document.querySelector("#example1");
            tableEl.innerHTML = "";

            // === HEADER ===
            const thead = document.createElement("thead");
            thead.style.backgroundColor = "#11C2D7";

            const headerRow = document.createElement("tr");

            // fixed headers
            const baseHeaders = ["", "Nama", "Item", "Satuan", "Jumlah", "HPS"];
            ...
            data.vendors.forEach(vendor => {
                ...
                thNama.textContent = vendor.nama || "";
                headerRow.appendChild(thNama);

                let thKet = document.createElement("th");
                thKet.textContent = vendor.Keterangan || "";
                headerRow.appendChild(thKet);
            });
            ...
            data.hps.forEach((hpsItem, i) => {
                ...
                btn.innerHTML = '<span class="fa fa-sort s-sort-symbol"></span>';
                ...
                tdJudul.textContent = (hpsItem.level == 0 ? hpsItem.judul : "") || "";
                tr.appendChild(tdJudul);

                // item
                const tdItem = document.createElement("td");
                tdItem.textContent = hpsItem.item || "";
                tr.appendChild(tdItem);

                // satuan
                const tdSatuan = document.createElement("td");
                tdSatuan.textContent = hpsItem.satuan || "";
                tr.appendChild(tdSatuan);

                // jumlah
                const tdJumlah = document.createElement("td");
                if (hpsItem.jumlah > 0) {
                    tdJumlah.textContent = accounting.formatNumber(hpsItem.jumlah, {
                        thousand: ".",
                        decimal: ",",
                        precision: 2
                    });
                ...
                tdHarga.className = "rata_kanan";
                if (hpsItem.harga > 0) {
                    tdHarga.textContent = accounting.formatNumber(hpsItem.harga, {
                        thousand: ".",
                        decimal: ",",
                        precision: 2
                    });
                ...
                // vendors
                data.vendors.forEach(vendor => {
                    const totItemInVendor = vendor.items.length;

                    const tdVendorHarga = document.createElement("td");
                    tdVendorHarga.className = "rata_kanan";
                    const tdVendorKet = document.createElement("td");

                    if (i < totItemInVendor) {
                        if (hpsItem.harga > 0 && vendor.items[i].harga > 0) {
                            tdVendorHarga.textContent = accounting.formatNumber(vendor.items[i].harga, { thousand: "." });
                        }
                        tdVendorKet.textContent = vendor.items[i].Keterangan || "";
                    }

                    tr.appendChild(tdVendorHarga);
                    tr.appendChild(tdVendorKet);
                });
...
function setListHargaVendor(vendor) {
    ...
    for (var i in vendor) { 
        str_vendor += '<div class="col-md-2"><div class="box box-primary box-folder-vendor"><div class="box-tools pull-right vendor-check-box"><div class="row" style="margin-right:3px;"><span class="fa fa-eye"></span> ' +
            ...
            '<p class="text-muted text-center deskripsi">Nilai Kriteria ' + DOMPurify.sanitize(vendor[i].NIlaiKriteria) + '</p>' +
                            '</div></div></div>';
    }

...
async function savedetailbudget(tmpPengadaanId, tmpBranch, tmpDepartment, tmpCOA, tmpInput, HeadId, tmpMonth) {
    var url = "api/Budget/AddDetailCOA"
    $.ajax({
        data: { pengadaanid: tmpPengadaanId, branch: tmpBranch, department: tmpDepartment, coa: tmpCOA, input: tmpInput, headerid: HeadId, month: tmpMonth },
        url: url,
        success: function (data) {
            $("#modal-load-coa").modal('hide');
            table.ajax.reload();
            //alert(data);
        },
        complete: function () {
...
function saveawal(pengadaanHeader, attr1) {
    ...
    $.ajax({
        ...
        if (attr1 === "showmodal") {
            ...
            $("#format-rks-asuransi").attr("href", "rks_asuransi.html#" + data.Id);
...
function save(pengadaanHeader, attr1, status) {
    ...
    $.ajax({
        ...
        if (attr1 == "showmodal") {
            $("#format-rks-asuransi").attr("href", "rks_asuransi.html#" + data.Id);
            $("#hpsmodal-list").modal("show"); //end asuransi
        } else {

            if (data.status == 200) {
                window.location.hash = encodeURIComponent(data.Id);
            ...
            if (status === "Draft") {
                $("#TitleKonfirmasi").text("Info");
                $("#KontenConfirmasi").text(data.message);
                $("#konfirmasi").modal("show");
    ...
    if ($("[name=AturanPenawaran]").val() === "Open Price") {
        ...
        $.ajax({
            method: "POST",
            url: "Api/PengadaanE/saveTotalHps?Id=" + encodeURIComponent($("#pengadaanId").val()) + "&Total=" + encodeURIComponent(totalHps),
            dataType: "json",
            data: JSON.stringify(data),
            headers: headers,
            contentType: 'application/json; charset=utf-8'
...
function getHeaderPengadaan() {
    ...
    viewPengadaan.CoaId = $("[name=CoaId]").val();
    ...
    viewPengadaan.Branch = $("[name=Branch]").val();
    viewPengadaan.Department = $("[name=Department]").val();
...
function loadHeaderPengadaan(viewPengadaan) {
    ...
    $("[name=Branch]").val(viewPengadaan.Branch);
    $("[name=Department]").val(viewPengadaan.Department);
    $("#pengadaanId").val(viewPengadaan.Id);
    //$('#pengadaan-langsung').val(viewPengadaan.PengadaanLangsung);
    if (viewPengadaan.PengadaanLangsung == 1)
    {
    ...
    //if ($("#pengadaanId").val() != null || $("#pengadaanId").val() != "") {
    //    var branch = $("[name=Branch]").val();
    //    //alert(branch);
    //    SetListDepartmentWithBranch("[name=Department]", branch);
    //}
...
//    $.ajax({
//        url: "/api/ReferenceData/GetAllDepartment?branch=" + branch,
//        success: function (data) {
//            for (var i in data) {
//                $(el).append("<option value='" + data[i].Department_Name + "'>" + data[i].Department_Name + "</option>");
//            }
//        }
//    });
//}
...
$(function () {
    ...
    SetListBranch("[name=Branch]");
    SetListDepartment("[name=Department]");
    

    SetListBranch("#listBranchCOA");
    SetListDepartment("#listDepartmentCOA");
    //SetListDepartmentWithBranch("[name=Department]");
    ...
    $("[name=Branch]").change(function () {
        ...
        url: "/api/ReferenceData/GetAllDepartment?branch=" + branch,
            success: function (data) {
                ...
                for (var i in data) {
                    $("[name=Department]").append("<option value='" + DOMPurify.sanitize(data[i].Department_Name) + "'>" + DOMPurify.sanitize(data[i].Department_Name) + "</option>");
                }
            }
        });
    });
    ...
    $("#anjukan-lanjutkan").on("click", function () {
        ...
        if ($("[name=HpsId]").val() > $("[name=CoaId]").val() && $("[name=CoaId]").val() != 0) {
            alert('Nilai HPS melebihi Anggaran Tersedia!');
            //window.location.reload();
        }
        //else if ($("[name=CoaId]").val() == 0) {
        //    save(getHeaderPengadaan(), "", "Ajukan");
        //}
        else save(getHeaderPengadaan(), "", "Ajukan");
    });
    ...
    $(".buat-baru").on("click", function () {
        window.location.replace(HOME_PAGE + window.location.pathname);
    });
    ...
    $.ajax({
        method: "post",
        url: "api/pengadaane/userinputpengadaan"
    }).done(function (data) {
        if (data == 1) {
            //alert(data);
            $(".user-hide").hide();
...
function renderDokumenDropzone(myDropzone, tipe) {
    const pengadaanId = $("#pengadaanId").val();
    if (!pengadaanId) {
        console.warn("pengadaanId kosong.");
        return;
    }

    $.ajax({
        url: "Api/PengadaanE/getDokumens?Id=" + encodeURIComponent(pengadaanId) + "&tipe=" + encodeURIComponent(tipe),
        success: function (data) {
            if (!data || data.length === 0) {
                console.info("Tidak ada dokumen ditemukan.");
                return;
            }
            ...
            data.forEach(doc => {
                ...
                // Opsional: Tambahkan thumbnail
                // myDropzone.emit("thumbnail", file, "/uploads/" + file.name);
            });
        },
        error: function (errormessage) {
            console.error("Gagal memuat dokumen:", errormessage);
        }
    });
}
...
$(function () {
    ...
    $("#pilih_coa").on("click", function () {

        if ($("#pengadaanId").val() == "") {
            alert("Harap Simpan Terlebih Dahulu");
        }
        else {
            $("#modal-load-coa").modal("show");
            var pengadaanid = $("#pengadaanId").val() || '';
            var branch = $("[name=Branch]").val() || '';
            ...
            $(function () {
                table_list = $('#table-modal-budget').DataTable({
                    ...
                    "columns": [
                        ...
                        //{ "data": "Year" },
                        //{ "data": "Month" },
                        { "data": "BudgetAmount", "style" : "text-align: right;", render: $.fn.dataTable.render.number('.', ',', 2, '') },
                        //{ "data": "BudgetUsage", render: $.fn.dataTable.render.number('.', ',', 2, '') },
                        { "data": "BudgetLeft", render: $.fn.dataTable.render.number('.', ',', 2, '') },
                        { "data": "Inputbudget"/*, render: $.fn.dataTable.render.number('.', ',', 2, ''), "class": "rata_kanan_pad10"*/ },
                        //{ "data": "BudgetReserved", render: $.fn.dataTable.render.number('.', ',', 2, '') },
                        { "data": "BudgetReserved", render: $.fn.dataTable.render.number('.', ',', 2, '') },
                        { "data": null },
                        { "data": "Id" },
                        { "data": "Version", "visible": false },
                        { "data": "BudgetType", "visible": false },
                    ...
                    "columnDefs": [
                        ...
                        },
                        {
                            "render": function (data, type, row) {
                                return '<a href="javascript:viewDetail(' + row.id + ')">' + data + '</a>';
                            },
                            "targets": 1,
                            "orderable": true
                        },
                        {
                            "render": function (data, type, row) {
                                var a = '<input onfocus="onJumlahDiallokasikanInputFocus(this, ' + row.BudgetLeft + ')" onchange="onJumlahDiallokasikanInputChange(this, ' + row.BudgetLeft + ',' + row.BudgetReserved +', ' + row.Id +')" type="text" class="form-control iniclassinput" id="iniidinput' + row.Id + '" value="' + row.Inputbudget + '" disabled style="text-align: right" />';
                                //var b = a + $.fn.dataTable.render.number('.', ', ', 2, '');
                                return a;
                            },
                            "targets": 5,
                            "orderable": false
                        ...
                        {
                            "render": function (data, type, row) {
                                var a = '<input type="text" class="form-control " id="BudgetOnProcess' + row.Id + '" value="' + row.BudgetReserved + '" disabled style="text-align: right" />';
                                return a;
                            },
                            "targets": 6,
                            "orderable": false
                        ...
                        {
                            "render": function (data, type, row) {
                                //var a = '<td class="tengah text-bold" style="text-align: center;" id="SisaBudgetEproc' + row.Id + '" >'+ (row.BudgetLeft - row.Inputbudget) +'</td>';
                                var a = '<td><p id="SisaBudgetEproc' + row.Id + '">' + (row.BudgetLeft - row.Inputbudget - row.BudgetReserved) +'</p></td>'
                                //var a = '<input type="text" class="form-control " id="SisaBudgetEproc' + row.Id + '" value="' + (row.BudgetLeft - row.Inputbudget) + '" disabled style="text-align: right" />';
                                return a;
                            },
                            "targets": 7,
                            "orderable": false
            ...
            $('.iniclassinput').on('click', function (evt) {
                console.log(evt.source);
            });

            $('#table-modal-budget').on('select.dt', function (e, api, type, indexes) {
                console.log('Row selected');
                var data = table_list.rows().pluck().data()[indexes];
                //console.log(data.BudgetAmount);
                $(".selected").find(".iniclassinput").prop("disabled", false);
                $(".selected").find("input[type=text]").val();
                //var myEle2 = document.getElementById("SisaBudgetEproc" + data.Id);
                ////var myEle2 = document.getElementById("SisaBudgetEproc" + data.Id);
                //if (myEle2) {
                //    var a = document.getElementById("iniidinput" + data.Id).value;
                //    myEle2.innerHTML = data.BudgetLeft - data.BudgetReserved - a;
                //}
                calculateTotalInput();
            });

            table_list.on('deselect', function (e, dt, type, indexes) {
                if (type === 'row') {
                    console.log('Row deselected');
                    var data = table_list.rows().pluck().data()[indexes];
                    $('#iniidinput' + data.Id).prop("disabled", true);
                    console.log(data.BudgetAmount);
                    var myEle = document.getElementById("iniidinput" + data.Id);
                    if (myEle) {
                        document.getElementById("iniidinput" + data.Id).value = 0;
                    }
                    var myEle2 = document.getElementById("SisaBudgetEproc" + data.Id);
                    //var myEle2 = document.getElementById("SisaBudgetEproc" + data.Id);
                    if (myEle2) {
            ...
            function saveheaderbudget(TotalValue, tmpPengadaanId) {
                var HeadId = null;
                var url = "api/Budget/AddHeaderCOA"
                $.ajax({
                    data: { PengadaanId: tmpPengadaanId, TotalInput: TotalValue },
                    url: url,
                    success: function (data) {
                        coba(data);
                    }
            ...
            function coba(HeaderId) {
                var tblData = table_list.rows('.selected').data();
                var totalValue = 0;
                var tmpId, tmpBranch, tmpDepartment, tmpCOA, tmpInput, tmpMonth;
                var tmpPengadaanId = $("#pengadaanId").val();
                $.each(tblData, function (i, val) {
                    tmpId = tblData[i].Id;
                    //TotalValue = $("#iniidinput" + tmpId).val();
                    totalValue += parseFloat($("#iniidinput" + tmpId).val());
                });
                console.log(totalValue);
                $.each(tblData, function (i, val) {
                    tmpId = tblData[i].Id;
                    tmpBranch = tblData[i].Branch;
                    tmpMonth = tblData[i].Month
                    tmpDepartment = tblData[i].Department;
                    tmpCOA = tblData[i].COA;
                    tmpInput = $("#iniidinput" + tmpId).val();
            ...
            $('#btnSelectedRows').on('click', function () {
                ...
                var tmpId, tmpBranch, tmpDepartment, tmpCOA, tmpInput;
                var tmpPengadaanId = $("#pengadaanId").val();
                var hpsAmount = $("[name=HpsId]").val();
                $.each(tblData, function (i, val) {
                    totalAllocated += parseFloat($("#iniidinput" + tblData[i].Id).val())
                });
                if (totalAllocated > Number(hpsAmount)) {
                    alert('Total Jumlah Dialokasikan Tidak Sama Dengan Nilai HPS!')
                }
                else {
                    var budgetAllocDetails = [];
                    $.each(tblData, function (i, val) {
                        budgetAllocDetails.push(
                            {
                                Branch: tblData[i].Branch,
                                Department: tblData[i].Department,
                                COA: tblData[i].COA,
                                Amount: $("#iniidinput" + tblData[i].Id).val(),
                                Month: tblData[i].Month,
                                Year: tblData[i].Year,
                                BudgetType: tblData[i].BudgetType,
                                Version: tblData[i].Version
                    ...
                    var postData = {
                        PengadaanId: tmpPengadaanId,
                        TotalAllocatedAmount: totalAllocated,
                        BudgetAllocDetails: budgetAllocDetails
                    }
                    console.log(postData);
                    var url = "api/Budget/SaveAllocation"
                    $.ajax({
                        method: 'POST',
                        data: postData,
                        url: url,
                        success: function (data) {
                            console.log('success simpan budget allocation', data);
                            $("#modal-load-coa").modal('hide');
                            window.location.reload();
                        },
                        error: function (err) {
                            console.error('Gagal simpan budget allocation', err);
                            alert('Gagal menyimpan data. ' + err.message);
                        },
                        complete: function () {
                            console.log('Simpan budget allocation selesai');
                        }
                    });
            ...
            //    //var data = $(this).closest("tr").removeAttr("disabled");
            //    //$(table_list.row(this).node()).filter('.iniclassinput').removeAttr("disabled");
            //    //var uhu = $(table_list.row(this).node()).filter('.iniclassinput').removeAttr("disabled");
            //    //var rowDariSellYgDiKlik = table_list.row(this).data();
            //    //console.log(rowDariSellYgDiKlik.data());
            //    //var uhu = $(table_list.row(this).node()).filter('.iniclassinput').val();
            //    //console.log(uhu);

            //});

            //$('#table-modal-budget tbody').on('click', 'td', function () {
            //    var cellYgDiKlik = table_list.cell(this);
            //    var rowDariSellYgDiKlik = table_list.row(this);
            //    var childDariSellYgDiKlik = table_list.row(this).child();
            //    console.log(cellYgDiKlik.data());
            //    console.log(rowDariSellYgDiKlik.data());
            //    console.log(childDariSellYgDiKlik);
            //    console.log($(rowDariSellYgDiKlik.node()))
            //    $(rowDariSellYgDiKlik.node()).filter('.iniclassinput').prop('disabled', false);

            //});

            //$('#table-modal-budget').on("click", ".harga-inputan", function () {
        ...
        $('#total-input').text(accounting.formatNumber($("[name=CoaId]").val(), 0, ".", ","));
    });

    var pengadaanId = $("#pengadaanId").val();

    if (!pengadaanId) {
        console.warn("pengadaanId kosong, AJAX tidak dijalankan.");
        return; // hentikan proses
    }

    $.ajax({
        url: "Api/PengadaanE/getTotalCOA?Id=" + encodeURIComponent(pengadaanId),
        method: "GET"
    }).done(function (data) {
        if (data.TotalInput)
            $("[name=CoaId]").val(data.TotalInput);
    });
...
//$(function () {
//    $.ajax({
//        url: "Api/PengadaanE/getTotalCOA?Id=" + $("#pengadaanId").val(),
//        method: "GET"
//    }).done(function (data) {
//        if (data.TotalInput != null || data.TotalInput != "")
//            $("[name=CoaId]").val(data.TotalInput);
//    });
//});
...
function calculateTotalInput() {
    ...
    //$('#total-input').text(accounting.formatMoney(totalValue, "Rp ", 0));
    $('#total-input').text(accounting.formatNumber(totalValue, 0, ".", ","));
}


function onJumlahDiallokasikanInputChange(input, budget, BudgetReserved, Id) {
    $('#table-modal-budget tr.selected').each(function (index, item) {
        //console.log(index, item);
        //console.log(input);
        var entryamount = $(input).val();
        var budgetleft = budget;
        var BudgetOnProcess = BudgetReserved;
        document.getElementById("SisaBudgetEproc" + Id).innerHTML = budgetleft - BudgetOnProcess - entryamount;

        //$(item).find('td input.iniclassinput ').each(function (i, t) {
        //    sum += Number($(t).val());
        //    console.log(i, $(t).val());
        //});

        console.log(entryamount, budgetleft, Id);
        if (entryamount > (budgetleft - BudgetOnProcess)) {
            alert('Jumlah dialokasikan harus kurang dari budget yang tersisa');
            var prev = $(input).data('val');
            $(input).val(prev);
            document.getElementById("SisaBudgetEproc" + Id).innerHTML = budgetleft - BudgetOnProcess - prev;
            //document.getElementById("SisaBudgetEproc" + Id).value = budgetleft - entryamount;
        }
    });

...
namespace Reston.Identity.Helper.Util
{
    public static class EndpointAndTokenHelper
    {
        public async static Task<JObject> CallUserInfoEndpoint(string accessToken)
        {
            ...
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(IdLdapConstants.AuthenticationHeaderValue.Bearer, accessToken);

            var response = await client.GetAsync(IdLdapConstants.Id.UserInfoEndpoint);

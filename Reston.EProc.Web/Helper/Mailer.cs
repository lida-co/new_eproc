...
namespace Reston.Pinata.WebService.Helper
{
    public class Mailer
    {
        ...
        public static bool sendText2(string ToName, string ToEmail, string BccEmail, string subject, string content)
        {
            return sendText2(ToName, ToEmail, BccEmail, FROM_NAME, FROM_EMAIL, subject, content);
        }
        ...
        public static bool sendText(string ToName, string ToEmail, string FromName, string FromEmail, string Subject, string Content)
        {
            ...
            )
            {
                ...
                try
                {
                    ...
                    System.IO.File.AppendAllText(path, "Email Penerima " + message.To + Environment.NewLine);
        ...
        public static bool sendText2(string ToName, string ToEmail, string BccEmail, string FromName, string FromEmail, string Subject, string Content)
        {
            ...
            MailAddress from = new MailAddress(FromEmail, FromName);
            MailAddress to = new MailAddress(ToEmail, ToName);
            //MailAddress copy = new MailAddress(BccEmail, BccName);
            ...
            )
            {
                ...
                foreach (var item in semuapersonel)
                {
                    message.CC.Add(item);
                }
                ...
                try
                {
                    ...
                    System.IO.File.AppendAllText(path, Environment.NewLine + DateTime.Now.ToString() + Environment.NewLine);
                    System.IO.File.AppendAllText(path, "Email Pengirim " + FROM_EMAIL + Environment.NewLine);
                    System.IO.File.AppendAllText(path, "Email Penerima " + message.To + message.CC + message.Bcc + Environment.NewLine);
                    System.IO.File.AppendAllText(path, "Sukses Terkirim" + Environment.NewLine);
